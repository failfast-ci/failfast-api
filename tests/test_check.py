...
def test_checkstatus_build_title(build_hook_data):
    check = CheckStatus(build_hook_data)
    assert check.check_title() == "Build Success"


def test_checkstatus_build_summary(build_hook_data):
    check = CheckStatus(build_hook_data)

    assert re.match(r"<img src='https://ffci-pub.s3.eu-central-1.amazonaws.com/icons/happy.*\.png'  height='64px' style='max-width:100%;vertical-align: -7px;'/>  The <a href='https://gitlab.com/failfast-ci/failfast-ci_failfast-api/builds/92764974'>Build</a> \*\*Succeeded\*\*.",
                    check.check_summary())
