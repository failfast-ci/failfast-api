...
class GithubClient(object):
    def __init__(self, installation_id):
        ...
        self._integration_pem_b64 = GConfig().github.integration_pem
        self._integration_id = GConfig().github.integration_id

    @property
    def integration_pem(self):
        if self._integration_pem is None:
            self._integration_pem = base64.b64decode(self._integration_pem_b64)
    ...
    def token(self):
        if not self._token:
            headers = {
                ...
                'User-Agent':
                    "ffci: %s" % ffci.__version__,
                'Authorization':
                    "Bearer %s" % jwt_token(self._integration_id,
