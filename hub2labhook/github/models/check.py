...
from hub2labhook.github.client import (
    GITHUB_CHECK_MAP,
    GITHUB_CHECK_ICONS,
    GITHUB_STATUS_MAP,
)
...
class CheckStatus(object):
    ...
    def duration(cls, started_at, finished_at):
        ...
        start_date, start_time = re.match(
            r"(.+)[ T](\d{2}:\d{2}:\d{2})", started_at
        ).groups()
        end_date, end_time = re.match(
            r"(.+)[ T](\d{2}:\d{2}:\d{2})", finished_at
        ).groups()
        begin = datetime.strptime(
            "%sT%s" % (start_date, start_time), "%Y-%m-%dT%H:%M:%S"
        )
        end = datetime.strptime("%sT%s" % (end_date, end_time), "%Y-%m-%dT%H:%M:%S")
    ...
    def check_name(self):
        if self.object_kind == "build":
            return "%s - %s" % (FFCONFIG.github["context"], self.object["build_name"])
        else:
            if self.ischild():
    ...
    def gitlab_status(self):
        if self.object_kind == "build":
            ...
            if (
                self.object["build_status"] == "failed"
                and self.object["build_allow_failure"] is True
            ):
...
{build_row}
""".format(
            build_row=self.build_info_row(build_info)
        )
    ...
    def check_pipeline_summary(self):
        ...
        text = (
            "<img src='{build_icon}'  height='64px' style='max-width:100%;vertical-align: -7px;'/>  "
            "The <a href='{build_url}'>Pipeline</a> {build_status}."
        ).format(
            build_url=self.details_url,
            build_icon=GITHUB_CHECK_ICONS[self.gitlab_status],
            build_status=title_map[self.gitlab_status],
    ...
    def check_pipeline_text(self):
        ...
        status = ("<img src='{build_icon}' height='16px'/> {status}").format(
            build_icon=GITHUB_CHECK_ICONS[icon], status=title_map[self.gitlab_status]
        )
        text = """
## Pipeline info
